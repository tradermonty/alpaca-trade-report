...
def main():
    ...
    try:
        if args.strategy:
            ...
            print(f"\n戦略 {args.strategy} の目標投資額: ${target_value:,.2f}")
        else:
            # 全戦略のサマリーを表示
            summary = get_allocation_summary(account=args.account)
            
            print(f"\n=== 戦略アロケーションサマリー ===")
            print(f"ポートフォリオ総額: ${summary['portfolio_value']:,.2f}")
            print(f"市場トレンド: {summary['market_trend']}")
            print(f"\n戦略別配分:")
            
            for strategy, info in summary['allocations'].items():
                print(f"  {strategy}: {info['ratio']:.1%} (${info['amount']:,.2f})")
