...
class TradeReport:
    ...
    def get_earnings_data(self):
        ...
        try:
            # FMPから決算カレンダーを取得
            earnings_data = self.fmp_fetcher.get_earnings_calendar(
                from_date=self.start_date,
                to_date=self.end_date
            )
            
            print(f"決算データ取得完了: {len(earnings_data)}件")
            
            # 従来形式に合わせてフォーマット
            combined_data = {'earnings': earnings_data}
    ...
    def get_historical_data(self, symbol: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        ...
        """
        try:
            print(f"株価データ取得開始: {symbol}")
            print(f"期間: {start_date} から {end_date}")
            
            # FMPから履歴データを取得
            historical_data = self.fmp_fetcher.get_historical_price_data(
                symbol=symbol,
        ...
        try:
            ...
            for e in earnings_data:
                ...
                if len(quarters) == 0 or (quarters[-1]['date'] - quarter_date).days > 60:
                    quarters.append({
                        'date': quarter_date,
                        'eps': float(e.get('epsActual', 0)) if e.get('epsActual') is not None else None,
                        'estimate': float(e.get('epsEstimate', 0)) if e.get('epsEstimate') is not None else None
                    })
    ...
    def get_market_cap(self, symbol):
        """FMPから会社プロファイルを取得して時価総額を算出"""
        try:
            # FMPから会社プロファイルを取得
            profile = self.fmp_fetcher.get_company_profile(symbol)
            
            if profile and 'mktCap' in profile:
                return float(profile['mktCap'])
            return None
    ...
    def get_previous_close(self, symbol: str, date: datetime.date) -> float:
        ...
        Args:
            symbol (str): 銘柄コード（例: AAPL, BF.B など）
        ...
        try:
            # 5日前からのデータを取得
            from_date = (date - pd.Timedelta(days=5)).strftime('%Y-%m-%d')
            ...
            if historical_data:
                # 指定日より前の最新の終値を探す
                for price_data in reversed(historical_data):
                    price_date = pd.to_datetime(price_data['date']).date()
                    if price_date < date:
                        return float(price_data['adjClose'])
