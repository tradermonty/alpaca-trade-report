...
class TradeReport:
    ...
    def get_previous_close(self, symbol: str, date: datetime.date) -> float:
        ...
        try:
            ...
            # 残りのドットをハイフンに変換（例：BF.B -> BF-B）
            api_symbol = base_symbol.replace('.', '-')
            # .USを付与
            api_symbol = f"{api_symbol}.US"
            ...
            params = {
                "api_token": self.api_key,
                "from": (date - pd.Timedelta(days=5)).strftime('%Y-%m-%d'),  # 5日前からのデータを取得
            ...
            if isinstance(data, list) and len(data) > 0:
                for price_data in reversed(data):
